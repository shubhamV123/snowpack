...
function filterExternalPackagesForEsm(externalPackages: string[]) {
  return externalPackages
    .map((spec) => [spec, resolveWebDependency(spec)] as [string, DependencyLoc])
    .filter(([spec, {type, loc}]) => type === 'JS')
    .filter(([spec, {type, loc}]) => {
      const code = fs.readFileSync(loc, 'utf8');
  ...
  const inputOptions: InputOptions = {
    ...
    plugins: [
      ...
      rollupPluginCommonjs({
        ...
        // @ts-ignore
        externalEsm: filterExternalPackagesForEsm(externalPackages),
      }),
